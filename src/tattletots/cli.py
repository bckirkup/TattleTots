...
def _load_scenario(
...
) -> GaussianShiftScenario | HighDimShiftScenario | SparseSensorScenario:
    ...
    if name == "sparse_sensor":
        return SparseSensorScenario.from_config(effective_config)
...
def _build_world(
    ...
    scenario: GaussianShiftScenario | HighDimShiftScenario | SparseSensorScenario,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--scenario",
        choices=["gaussian_shift", "high_dim_shift", "sparse_sensor"],
    ...
    print(f"  Uniform precision: {summary['chance_precision']:.2%}")
    print(f"  Static-prior precision: {summary['static_prior_precision']:.2%}")
