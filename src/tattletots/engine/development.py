...
def apply_mimesis(
    ...
    if model.genome.input_preference.size > 0:
        ...
        updated = _nudge_vector_toward(
            juvenile.state.input_preference_override,
            juvenile.genome.input_preference,
            target,
            lr,
        )
    ...
    if model.genome.fusion_weights.size > 0:
        ...
        updated_fw = _nudge_vector_toward(
            juvenile.state.fusion_weights_override,
            juvenile.genome.fusion_weights,
            target_fw,
            lr,
        )
        if updated_fw is not None:
            juvenile.state.fusion_weights_override = updated_fw
